...
on_opt_parse () {
    case "$1" in
        ...
        update)
            ...
            ytfzfctl update "$@"
            exit ;;
        comments) $0 -ccomments --thumb-viewer=null "$(ytfzfctl video-info url)"; exit ;;
        vi|info|video-info) $0 -cvideo-info --thumb-viewer=null "$(ytfzfctl video-info url)"; exit ;;
    esac
}
...
ext_on_search () {
    ...
    case "$1" in
        :exit) exit 0  ;;
        u://*) expr "$curr_scrape" : "u\|U\| url" > /dev/null || curr_scrape=U
            _search=${_search#u://}
            ;;
